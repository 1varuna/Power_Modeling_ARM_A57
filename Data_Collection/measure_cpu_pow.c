...
Top file containting all essential methods to read from CPU
...
void *a57_read_samples(void *head) {
	...
	//printf("\n\t measure_cpu_pow.c::a57_read_samples() : Writing first line in log FILE \n");
	if (ev_group==1)
	...
	while(1){
		...
		//if(sample_count%5==0)
		{
			...
			/* Adding some delay to prevent counters being 0 all the time during run */
			usleep(10000);			
			/* read_a57_counters('a') contains init_counters() */
			read_a57_counters('a');
